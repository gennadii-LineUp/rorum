...
@Component({
    selector: 'jhi-organisation-structure',
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/organisation-structure'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/organisation-structure', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrganisationStructure) {
        return item.id;
    }
    registerChangeInOrganisationStructures() {
        this.eventSubscriber = this.eventManager.subscribe('organisationStructureListModification', (response) => this.loadAll());
    }
    ...
    private onSuccess(data, headers) {
        ...
        this.organisationStructures = data;
    }
