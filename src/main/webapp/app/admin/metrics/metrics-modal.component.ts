...
export class JhiMetricsMonitoringModalComponent implements OnInit {
    ...
    ngOnInit() {
        this.threadDump.forEach((baseValue) => {
            if (baseValue.threadState === 'RUNNABLE') {
                this.threadDumpRunnable += 1;
            } else if (baseValue.threadState === 'WAITING') {
                this.threadDumpWaiting += 1;
            } else if (baseValue.threadState === 'TIMED_WAITING') {
                this.threadDumpTimedWaiting += 1;
            } else if (baseValue.threadState === 'BLOCKED') {
