    ...
    private addHealthObject(result, isLeaf, healthObject, name): any {
        ...
        for (const key in healthObject) {
            if (healthObject.hasOwnProperty(key)) {
                const baseValue = healthObject[key];
                if (key === 'status' || key === 'error') {
                    healthData[key] = baseValue;
                } else {
                    if (!this.isHealthObject(baseValue)) {
                        details[key] = baseValue;
                        hasDetails = true;
                    }
                }
            }
    ...
    private flattenHealthData(result, path, data): any {
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                ...
                if (this.isHealthObject(baseValue)) {
                    if (this.hasSubSystem(baseValue)) {
                        this.addHealthObject(result, false, baseValue, this.getModuleName(path, key));
                        this.flattenHealthData(result, this.getModuleName(path, key), baseValue);
                    } else {
                        this.addHealthObject(result, true, baseValue, this.getModuleName(path, key));
                    }
                }
            }
        }
